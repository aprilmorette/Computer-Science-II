...
public class MinesweeperModel {
	...
	// sets up game
	public void setupBoard() {  // populating the arrays with mines
		cellState = new int[10][10]; 
		board = new int[10][10];
		for (int row = 0; row < 10; row++) {
			for (int col = 0; col < 10; col++) { // probability of being a mine = 10%
				int rand = (int)(Math.random()*100) + 1;
				this.cellState[row][col] = (rand < 10) ? mineHidden : safeHidden; // assigns 1 if mine
	...
	public int getMineCount(int row, int col) {
		if (row == 0) {
			...
			return isMine(row-1, col-1) + isMine(row-1, col) + isMine(row-1, col+1) + isMine(row, col-1) + isMine(row, col+1) + isMine(row+1, col+1) + isMine(row+1, col) + isMine(row+1, row-1);
		}
	}
